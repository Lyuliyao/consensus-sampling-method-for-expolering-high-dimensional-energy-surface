...
class Adachange_F6 : public Bias {
  std::vector<float> at;
  ...
  std::ofstream mean_file,var_file,out_file;
  int seed;
  void get_at_file();
public:
  explicit Adachange_F6(const ActionOptions&);
  void calculate() override;
  static void registerKeywords(Keywords& keys);
};

PLUMED_REGISTER_ACTION(Adachange_F6,"ADACHANGE_F6")

void Adachange_F6::registerKeywords(Keywords& keys) {
  ...
  keys.add("compulsory","BETA","10","the inverse temperature of the collective variable");
  keys.add("compulsory","BETAA","10","the inverse temperature of the collective variable");
...
Adachange_F6::Adachange_F6(const ActionOptions&ao):
  PLUMED_BIAS_INIT(ao),
...
void Adachange_F6::calculate() {
  ...
  for(int i=0; i<N_CV; ++i) {
    ...
    if (time_now<equilibrium_time/2){
      const float f = -(0.1+1.8*time_now/equilibrium_time)*(k*cv+m);
  ...
  if (time_now%full_dynamic_time==0){
    ...
    value += (F_local[i])*(F_local[i]);
    }  
    err = -betaa*sqrt(err/value);
    ...
    float err_exp = exp(-beta*err);
    multi_sim_comm.Sum(err_exp);

    for (int i=0; i<N_CV; ++i) 
    E_CV[i] = exp(-beta*err)*X_CV_local[i]/err_exp;
    multi_sim_comm.Sum(&E_CV[0], N_CV);


    for (int i=0; i<N_CV; ++i) 
    Var_CV[i] = 2*beta*exp(-beta*err)*(X_CV_local[i]-E_CV[i])*(X_CV_local[i]-E_CV[i])/err_exp;
  ...
  // for (int j=0; j<N_CV; ++j)
  // F_CV[j] =  -(X_CV_local[j] - E_bar_CV_unbias[j])/(Var_bar_CV_unbias[j]+epsilon) -gamma*V_CV_local[j] + sqrt(2*gamma/(alpha*beta))*random[0].Gaussian();
  ...
  for (int j=0; j<N_CV; ++j){
    ...
    X_CV_local[j] = X_CV_local[j]  + sqrt(2*alpha/(gamma*beta))*random[0].Gaussian();
  }
