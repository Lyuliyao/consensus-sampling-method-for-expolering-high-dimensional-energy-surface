...
class Potential(torch.nn.Module):
    def __init__(self,N_in,N_w):
        ...
        self.W4 = torch.nn.Parameter(torch.randn(N_w,N_w)/5)
        self.b4 = torch.nn.Parameter(torch.randn(N_w,1)/5)  
        self.W5 = torch.nn.Parameter(torch.randn(1,N_w)/5)
        self.b5 = torch.nn.Parameter(torch.randn(1,1)/5)
    ...
    def forward(self, input):
        ...
        y3 = self.ac(y3)+y2;
        y4 = self.W4@y3 + self.b4;
        y4 = self.ac(y4)+y3;
        output = self.W5@y4 + self.b5;
        return output
...
def init_model(dim):
    potential = Potential(2*dim,1024);
    return potential
